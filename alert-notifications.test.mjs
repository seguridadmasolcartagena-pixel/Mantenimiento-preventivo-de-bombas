  ...
  await window.fetch("https://flow.example/save", {
    body: JSON.stringify({
      pumps: [{
        ...
        pumpType: "Centrífuga", aviso: "4", alarma: "6", cfPlusAviso: "11", cfPlusAlarma: "16",
...
assert.equal((await alertsFor({ cfPlus: 16 }))[0].status, "Alarma");
assert.equal((await alertsFor({ vibration: 6, cfPlus: 10 }))[0].alertMetric, "Vibración");
assert.equal((await alertsFor({ vibration: 6, cfPlus: 16 }))[0].alertMetric, "Vibración y CF+");
